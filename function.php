...
function register($data) {
    ...
    $no_kk = htmlspecialchars(strtolower($data['no_kk']));
    ...
    if ( empty($nama) || empty($no_kk)  || empty($password) || empty($pass_confir) ) {
        echo "<script>
    ...
    $nama_ada = "SELECT nama, no_kk FROM user WHERE no_kk = '$no_kk' || nama = '$nama' ";
    ...
    // Input Data
    $q_regis = "INSERT INTO user VALUES (null, '$nama', '$no_kk', '$pw_hash')";
...
function login($data) {
    ...
    $sama = "SELECT * FROM user WHERE no_kk = '$no_kk' ";
    $sama2 = mysqli_query($conn, $sama);
    $sama3 = "SELECT * FROM admin WHERE no_kk = '$no_kk' ";
